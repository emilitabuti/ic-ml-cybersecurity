...
def append_prediction_history(item: dict[str, str | float]) -> None:
    normalized = {
...
def remove_prediction_history_by_model(model: str) -> None:
    with _history_lock:
        remaining = [item for item in _history if item.get("model") != model]
        _history.clear()
